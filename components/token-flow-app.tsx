...
import type { PdfMode, ProviderId, SavedTemplate, TemplateCategory, ToolId } from "@/types";
import { cleanContext, downloadTextFile, optimizePrompt } from "@/utils/text";
import { estimateCost, estimateTokens, formatCurrency, MODEL_PRICING } from "@/utils/tokenizer";
...
const tools: Array<{ id: ToolId; title: string; description: string; accent: string }> = [
  ...
  },
  {
    id: "templates",
    title: "Templates",
    description: "Save reusable study, coding, and business prompts locally.",
    accent: "bg-[#d69961]"
  }
];
...
export function TokenFlowApp() {
  ...
  function saveTemplate() {
    ...
    if (editingTemplateId) {
      ...
      toast.show("Template saved locally");
    }
  ...
  return (
    <main className="min-h-screen overflow-hidden">
      ...
      <div className="mx-auto flex w-full max-w-7xl gap-6 px-6 py-8">
        <aside className="sticky top-8 hidden h-[calc(100vh-4rem)] w-72 shrink-0 flex-col justify-between rounded-[2.2rem] border border-white/80 bg-white/70 p-5 shadow-soft backdrop-blur lg:flex">
          <div>
            <div className="rounded-[1.8rem] bg-ink p-5 text-cream">
              <p className="text-xs font-bold uppercase tracking-[0.24em] text-cream/60">TokenFlow</p>
              <h1 className="mt-3 font-display text-4xl font-black leading-none">AI context, trimmed clean.</h1>
            </div>
          ...
          <div className="rounded-3xl bg-cream p-4 text-sm text-ink/65">
            Browser-first architecture: token estimates, cleaning, templates, and PDF parsing stay local where possible.
          </div>
        </aside>

        <section className="min-w-0 flex-1">
          <header className="mb-8 rounded-[2.5rem] border border-white/80 bg-white/65 p-7 shadow-soft backdrop-blur">
            <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
              <div>
                <p className="text-sm font-black uppercase tracking-[0.28em] text-leaf">Vercel-ready MVP</p>
                <h2 className="mt-4 max-w-3xl font-display text-5xl font-black leading-[0.95] text-ink md:text-7xl">
                  Spend fewer tokens. Finish in fewer prompts.
                </h2>
                <p className="mt-5 max-w-2xl text-lg leading-8 text-ink/68">
                  Optimize prompts, clean noisy context, estimate model costs, and convert PDFs into AI-ready Markdown.
                </p>
              </div>
              <div className="grid grid-cols-3 gap-3">
                <Metric label="Local-first" value="Free" tone="good" />
                <Metric label="Tools" value="5" />
                <Metric label="Storage" value="Browser" tone="warm" />
              </div>
            </div>
          </header>
          ...
          <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-5">
            {tools.map((tool) => (
              <button
                key={tool.id}
                onClick={() => setActiveTool(tool.id)}
                className={`group rounded-[1.6rem] border p-4 text-left transition hover:-translate-y-1 hover:shadow-card ${
          ...
          {activeTool === "calculator" && (
            <Card>
              <ToolHeading
                eyebrow="Phase 4"
                title="Token Usage Calculator"
                description="Estimate input tokens locally and compare approximate costs across GPT, Claude, and Gemini-style pricing."
              />
              <div className="grid gap-5 xl:grid-cols-[1fr_22rem]">
                ...
                <div className="rounded-3xl bg-cream p-5">
                  ...
                  <input
                    type="number"
                    min={0}
                    className="mt-3 w-full rounded-2xl border border-sand bg-white p-3 text-lg font-black"
                  ...
                  <p className="mt-4 text-sm leading-6 text-ink/58">
                    Costs are configurable estimates, not invoices. Swap values in `utils/tokenizer.ts` as pricing changes.
                  </p>
                </div>
              </div>
          ...
          {activeTool === "templates" && (
            <Card>
              <ToolHeading
                ...
                description="Save personal study, coding, and business prompts in browser storage. A database can replace this layer later."
              />
              <div className="grid gap-5 xl:grid-cols-[0.8fr_1.2fr]">
                <div className="space-y-3 rounded-[2rem] bg-cream p-5">
                  <FieldLabel>{editingTemplateId ? "Edit Template" : "New Template"}</FieldLabel>
                  <input
                    className="w-full rounded-2xl border border-sand bg-white p-3 font-bold"
                  ...
                  <TextArea
                    ...
                    placeholder="Paste your reusable prompt..."
                  />
                  <Button onClick={saveTemplate}>{editingTemplateId ? "Update Template" : "Save Template"}</Button>
                </div>
                <div className="grid gap-4">
                  {templates.length === 0 && (
                    <div className="rounded-[2rem] border border-dashed border-sand bg-white/70 p-8 text-center text-ink/58">
                      No templates yet. Save one and it will persist in this browser.
                    </div>
                  )}
                  {templates.map((template) => (
                    <article key={template.id} className="rounded-[2rem] border border-sand/70 bg-white p-5">
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          <p className="text-xs font-black uppercase tracking-[0.16em] text-clay">{template.category}</p>
                      ...
                      <p className="mt-4 whitespace-pre-wrap rounded-2xl bg-cream p-4 text-sm leading-6 text-ink/70">
                        {template.content}
                      </p>
                    </article>
                  ))}
